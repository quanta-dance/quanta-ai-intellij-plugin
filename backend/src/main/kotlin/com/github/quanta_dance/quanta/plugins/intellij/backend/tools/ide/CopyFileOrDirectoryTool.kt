...
class CopyFileOrDirectoryTool : ToolInterface<String> {
    ...
    override fun execute(project: Project): String {
        val base = PathUtils.projectRootPath(project) ?: return "Project base path not found."
        val source =
            try {
        ...
        return try {
            if (Files.isDirectory(source)) {
                ...
                val finalTarget =
                    if (Files.exists(destNorm) && Files.isDirectory(destNorm)) destNorm.resolve(source.fileName) else destNorm
