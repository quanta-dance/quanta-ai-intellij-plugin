...
data class OpenAIResponse
    ...
    constructor(
        ...
        @field:JsonPropertyDescription(
            "Optional reported plan goal text. When changing the persisted plan, call SessionPlanTool explicitly instead of relying on this field alone.",
        )
        val planGoal: String? = null,
        @param:JsonProperty("planDefinitionOfDone")
        @field:JsonPropertyDescription(
            "Optional reported plan definition of done text. When changing the persisted plan, call SessionPlanTool explicitly instead of relying on this field alone.",
        )
        val planDefinitionOfDone: String? = null,
        @param:JsonProperty("planTasks")
        @field:JsonPropertyDescription(
            "Optional reported full task list (without [ ] markers). When changing the persisted plan, call SessionPlanTool explicitly instead of relying on this field alone.",
        )
        val planTasks: List<String>? = null,
        @param:JsonProperty("planCompletedTasks")
        @field:JsonPropertyDescription(
            "Optional reported tasks completed in this turn. Persist them by calling SessionPlanTool explicitly; these fields alone do not update the stored session plan state.",
        )
