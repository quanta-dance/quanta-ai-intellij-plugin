...
class RequestModelSwitch : ToolInterface<Map<String, Any>> {
    ...
    override fun execute(project: Project): Map<String, Any> {
        ...
        val requestedRaw = (desiredModel ?: settings.model).ifBlank { settings.model }
        ...
        val runtimeModel = currentModel?.takeIf { it.isNotBlank() } ?: settings.model
        ...
        if (!dynamicEnabled) {
            ...
            if (rank(normRequested) > rank(normMax)) {
                ...
                chosen = normMax
                msg =
                    "Requested model ($normRequested) exceeds configured cap ($normMax); upgrade denied. Current: $normCurrent"
            } else {
                // within cap: approve
                approved = true
                chosen = capped
