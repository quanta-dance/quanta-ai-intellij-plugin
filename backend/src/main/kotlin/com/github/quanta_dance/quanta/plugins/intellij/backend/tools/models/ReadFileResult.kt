...
data class ReadFileResult(
    ...
    @field:JsonPropertyDescription(
        "Optional non-fatal note or warning for successful reads, for example when a requested range was clamped to file bounds.",
    )
    val warning: String = "",
    ...
    @field:JsonPropertyDescription(
        "True when the tool had to truncate or window the requested content instead of returning the full requested slice.",
    )
    val truncated: Boolean = false,
    @field:JsonPropertyDescription(
        "True when lines after actualToLine still remain in the requested or file content and were not returned.",
    )
