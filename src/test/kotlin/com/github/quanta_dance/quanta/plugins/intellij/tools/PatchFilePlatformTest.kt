...
class PatchFilePlatformTest : BasePlatformTestCase() {
    ...
    private fun currentVersion(file: PsiFile): Long {
        ...
        val vfsStamp = try { vf.modificationStamp } catch (_: Throwable) { 0L }
        return VersionUtil.computeVersion(psiStamp, docStamp, vfsStamp)
    }

    private fun createUnderProject(relativePath: String, content: String): PsiFile {
    ...
    fun testSkipMismatchesAppliesOthers() {
        ...
        val tool = PatchFile().apply {
            filePath = "src/D.kt"
            patches = listOf(
                PatchFile.Patch(fromLine = 2, toLine = 2, newContent = "TWO", expectedText = "MIS"), // mismatch
                PatchFile.Patch(fromLine = 4, toLine = 4, newContent = "FOUR", expectedText = "four") // ok
            )
    ...
    fun testStartLineBeyondDocumentSkippedWhenAllowed() {
        ...
        val tool = PatchFile().apply {
            filePath = "src/F.kt"
            patches = listOf(
                PatchFile.Patch(fromLine = 10, toLine = 10, newContent = "TEN"), // beyond doc
