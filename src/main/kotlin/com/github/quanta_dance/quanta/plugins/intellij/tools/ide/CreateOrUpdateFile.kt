...
class CreateOrUpdateFile : ToolInterface<String> {
    ...
    @field:JsonPropertyDescription("New content for the file to be modified. If provided and 'patches' is empty, this fully replaces file content.")
    ...
    @field:JsonPropertyDescription("Optional list of line-range patches to apply (1-based inclusive lines). If non-empty, patches are applied instead of full replace.")
    ...
    private fun flushPsiAndVfs(project: Project, target: VirtualFile?) {
        try {
            ...
            if (target != null) { try { VfsUtil.markDirtyAndRefresh(true, false, false, target) } catch (_: Throwable) {} }
            try { PsiDocumentManager.getInstance(project).commitAllDocuments() } catch (_: Throwable) {}
        } catch (_: Throwable) {}
    }
    ...
    override fun execute(project: Project): String {
        ...
        val resolved = try { PathUtils.resolveWithinProject(projectBase, filePath) } catch (e: IllegalArgumentException) {
        ...
        ApplicationManager.getApplication().invokeAndWait {
            WriteCommandAction.runWriteCommandAction(project) {
                try {
                    ...
                    val parentVf = try { VfsUtil.createDirectories(ioFile.parentFile.absolutePath) } catch (e: Throwable) {
                    ...
                    if (document != null) {
                        try { document.setText(content ?: ""); PsiDocumentManager.getInstance(project).commitDocument(document); FileDocumentManager.getInstance().saveDocument(document) } catch (e: Throwable) { QDLog.debug(logger) { "Failed to set/commit/save document: ${e.message}" } }
                    } else {
        ...
        try {
            if (validateBuildAfterUpdate) {
                ...
                PsiDocumentManager.getInstance(project).commitAllDocuments(); FileDocumentManager.getInstance().saveAllDocuments()
                val ioFile = resolved.toFile(); val vFile = LocalFileSystem.getInstance().refreshAndFindFileByIoFile(ioFile)
                if (vFile != null) { try { VfsUtil.markDirtyAndRefresh(true, true, true, vFile) } catch (_: Throwable) {}; updatedVirtualFile = vFile; try { PsiDocumentManager.getInstance(project).commitAllDocuments() } catch (_: Throwable) {} }
            }
        } catch (e: Throwable) { QDLog.debug(logger) { "Post-write sync failed: ${e.message}" } }

        if (validateAfterUpdate) { result += "\n" + runPsiValidation(project, relToBase) }
        QDLog.info(logger) { "Update file $relToBase: $result, file version: $lastModified" }
        return result
    }

    private fun runPsiValidation(project: Project, relToBase: String): String {
        return try {
            ...
            summary.lines().first()
        } catch (e: Throwable) { QDLog.warn(logger, { "Validation unavailable for $relToBase" }, e); "Validation: skipped (${e.message})" }
    }
}
