...
import com.github.quanta_dance.quanta.plugins.intellij.services.SessionSchedulerService
...
class ScheduleTaskTool : ToolInterface<Map<String, Any>> {
    ...
    @field:JsonPropertyDescription(
        "Optional agent id that owns this schedule. When set, scheduler enforces at most one pending one-shot job per agent.",
    )
    ...
    override fun execute(project: Project): Map<String, Any> {
        ...
        return when (act) {
            ...
            "ADD" -> {
                ...
                val owner = agentId?.trim().orEmpty()
                if (owner.isBlank()) {
                    return mapOf(
                        "status" to "error",
                        "message" to "agentId is required for ADD (one one-time schedule per agent).",
                    )
                }
