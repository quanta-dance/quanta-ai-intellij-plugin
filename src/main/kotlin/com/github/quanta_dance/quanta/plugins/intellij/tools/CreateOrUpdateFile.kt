...
class CreateOrUpdateFile : ToolInterface<String> {
    ...
    // Keep default true, but do not run Gradle; instead force a synchronous PSI/VFS flush before validation
    ...
    private fun flushPsiAndVfs(project: Project, target: VirtualFile?) {
        try {
            // Save all and commit repeatedly until no uncommitted docs remain (bounded attempts)
            var attempts = 0
            while (attempts < 3) {
            ...
            // Refresh VFS for the target file
            if (target != null) {
                try { VfsUtil.markDirtyAndRefresh(true, false, false, target) } catch (_: Throwable) {}
            }
            // Final commit pass
            try { PsiDocumentManager.getInstance(project).commitAllDocuments() } catch (_: Throwable) {}
        } catch (_: Throwable) {}
    ...
    override fun execute(project: Project): String {
        ...
        if (patchList.isNotEmpty()) {
            ...
            if (validateBuildAfterUpdate) {
                // Force immediate PSI/VFS flush so parse errors appear synchronously in our validator
            ...
            // Run PSI validation if requested
        ...
        ApplicationManager.getApplication().invokeAndWait {
            WriteCommandAction.runWriteCommandAction(project) {
                try {
                    ...
                    val parentVf =
                        try { VfsUtil.createDirectories(ioFile.parentFile.absolutePath) } catch (e: Throwable) {
                    ...
                    if (document != null) {
                        ...
                        try { virtualFile.setBinaryContent((content ?: "").toByteArray(StandardCharsets.UTF_8)) } catch (e: Throwable) {
