...
class ReadFileContent : ToolInterface<ReadFileResult> {
    ...
    @field:JsonPropertyDescription("If true, and the file is the current editor file with caret/selection, return a window around caret/selection when truncating.")
    ...
    companion object { private val logger = Logger.getInstance(ReadFileContent::class.java) }
    ...
    private fun withLineNumbers(fileContent: String, startLineNumber: Int = 1): String {
    ...
    private fun clampWindow(start: Int, end: Int, lineCount: Int): Pair<Int, Int> {
        val s = start.coerceAtLeast(0); val e = end.coerceAtMost(lineCount - 1).coerceAtLeast(s)
        return Pair(s, e)
    }

    private fun sliceByLines(raw: String, fromLine0: Int, toLine0: Int): String {
        val lines = raw.lines(); val (s, e) = clampWindow(fromLine0, toLine0, lines.size)
        return lines.subList(s, e + 1).joinToString("\n")
    }

    private fun headByLines(raw: String, maxChars: Int): Pair<String, Int> {
        val lines = raw.lines(); val out = StringBuilder(); var count = 0; var i = 0
        while (i < lines.size) {
            val add = if (out.isEmpty()) lines[i] else "\n" + lines[i]
            if (out.length + add.length > maxChars) break
            out.append(add); count++; i++
        }
        return Pair(out.toString(), 0)
    }

    private fun tailByLines(raw: String, maxChars: Int): Pair<String, Int> {
        val lines = raw.lines(); val out = StringBuilder(); var count = 0; var i = lines.lastIndex; val stack = ArrayDeque<String>()
        while (i >= 0) {
            val add = if (stack.isEmpty()) lines[i] else lines[i] + "\n"
            if (out.length + add.length > maxChars) break
            stack.addFirst(lines[i]); out.append(add); count++; i--
        }
    ...
    override fun execute(project: Project): ReadFileResult {
        ...
        val resolved = try { PathUtils.resolveWithinProject(basePath, filePath) } catch (e: IllegalArgumentException) {
            addMsg(project, "Read file content - rejected", e.message ?: "Invalid path"); return ReadFileResult("", "", e.message ?: "Invalid path")
        }
        ...
        return ApplicationManager.getApplication().runReadAction<ReadFileResult> {
            ...
            val virtualFile = baseDir?.findFileByRelativePath(relToBase) ?: return@runReadAction ReadFileResult("", "", "File not found.")
            ...
            val currentCtx = try { CurrentFileContextProvider(project).getCurrent() } catch (_: Throwable) { null }
            val isCurrentTarget = currentCtx?.filePathRelative?.equals(relToBase, ignoreCase = false) == true

            var finalContent = rawContent; var truncated = false; var firstLineNumber = 1
            if (isTooLarge) {
                val lines = rawContent.lines(); val lc = lines.size
                when (strategy.lowercase()) {
                    "head" -> { val (slice, start0) = headByLines(rawContent, maxChars); finalContent = slice; truncated = true; firstLineNumber = start0 + 1 }
                    "tail" -> { val (slice, start0) = tailByLines(rawContent, maxChars); finalContent = slice; truncated = true; firstLineNumber = start0 + 1 }
                    else -> { // window
                        if (preferWindowIfCurrentFile && isCurrentTarget) {
                            val cStart = currentCtx?.selectionStartLine; val cEnd = currentCtx?.selectionEndLine; val caret = currentCtx?.caretLine
                            val startLine0: Int; val endLine0: Int
                            if (cStart != null && cEnd != null) {
                                ...
                                val (slice, start0) = headByLines(rawContent, maxChars); finalContent = slice; truncated = true; firstLineNumber = start0 + 1; startLine0 = 0; endLine0 = -1
                            }
                            if (startLine0 <= endLine0) { finalContent = sliceByLines(rawContent, startLine0, endLine0); truncated = true; firstLineNumber = startLine0 + 1 }
                        } else { val (slice, start0) = headByLines(rawContent, maxChars); finalContent = slice; truncated = true; firstLineNumber = start0 + 1 }
                    }
                }
            }
