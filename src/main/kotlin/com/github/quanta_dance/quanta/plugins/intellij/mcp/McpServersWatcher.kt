...
class McpServersWatcher(private val project: Project) : Disposable {
    ...
    private val configPathSi: String? = McpServersPaths.resolve(project)
        ?.toAbsolutePath()
        ?.normalize()
    ...
    init {
        ...
        if (configPathSi.isNullOrEmpty()) {
            val msg =
                "MCP servers watcher could not resolve config path (project base: ${project.basePath ?: "<null>"}). No file changes will be observed."
            QDLog.warn(log) { msg }
        } else {
