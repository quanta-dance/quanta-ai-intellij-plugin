...
class RunGoTestsTool : ToolInterface<RunGoTestsTool.Result> {
    ...
    @JsonPropertyDescription("Working directory relative to the project root (default: project root). If autoDetectModule=true and go.mod exists in project root, it will be used.")
    ...
    @JsonPropertyDescription("Absolute path to go binary (optional). If omitted, we try GOROOT/bin/go, common locations, or '/usr/bin/env go'.")
    ...
    private fun canInvoke(cmd: Array<String>, wd: File? = null): Boolean {
        return try {
    ...
    override fun execute(project: Project): Result {
        ...
        val workDir = run {
            ...
            if (!autoDetectModule) File(basePath, userWd ?: ".")
            else {
        ...
        val toolMsg = try { project.service<ToolWindowService>().startToolingMessage("Run Go tests", taskLine) } catch (_: Throwable) { null }
        val spinner = try { project.service<ToolWindowService>().startSpinner("Running go test…") } catch (_: Throwable) { null }
        ...
        process.inputStream.bufferedReader(StandardCharsets.UTF_8).use { reader ->
            ...
            while (reader.readLine().also { line = it } != null) {
                ...
                val t = ln.trim()
                when {
                    t.startsWith("ok\t") -> { totalPkgs += 1; pushRecent(t); pulse() }
                    t.startsWith("FAIL\t") -> { totalPkgs += 1; failedPkgs += 1; pushRecent(t); pulse() }
                    t.startsWith("?\t") -> { totalPkgs += 1; pushRecent(t); pulse() }
                    t.startsWith("--- FAIL:") || t.startsWith("--- PASS:") || t.startsWith("=== RUN") -> { pushRecent(t); pulse() }
                }
            }
        }
        ...
        if (!finished) {
            ...
            return Result(false, totalPkgs, failedPkgs, tail(output, stdoutTailLines), "go test timed out")
        }
        ...
        return Result(success, totalPkgs, failedPkgs, tail(output, stdoutTailLines), if (success) "" else "go test failed with exit code $exitCode")
    }

    private fun tail(sb: StringBuilder, n: Int): String? {
        if (n <= 0) return null
