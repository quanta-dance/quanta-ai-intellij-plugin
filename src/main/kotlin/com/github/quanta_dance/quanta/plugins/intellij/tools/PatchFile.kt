...
class PatchFile : ToolInterface<String> {
    ...
    data class Patch(
        ...
        @JsonPropertyDescription("Optional expected current text for the specified line range. If provided and does not match, patch is skipped or triggers failure depending on stopOnMismatch.")
    ...
    @JsonPropertyDescription("If true (default), aborts and applies nothing when any patch guard fails. If false, skips only mismatched patches and applies the rest.")
    var stopOnMismatch: Boolean = true

    @JsonPropertyDescription("Optional expected file version (PSI/Document/VFS) before patching. If present and does not match, no changes are applied.")
    var expectedFileVersion: Long? = null

    companion object { private val logger = Logger.getInstance(PatchFile::class.java) }
                    ...
                    if (stopOnMismatch) {
                        ...
                        if (mismatches.isNotEmpty()) {
                            result.append("Patched 0 range(s) in ").append(relToBase)
                                .append(" with ").append(mismatches.size).append(" mismatch(es). Aborted due to stopOnMismatch=true. ")
